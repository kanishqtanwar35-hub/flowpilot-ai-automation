# Practical Technical Test â€” AI Automation Developer

**Candidate:** Kanishq
**Company:** I Vision Infotech
**Submission:** FlowPilot â€” an inbound lead & support triage automation, plus its control-room
dashboard. One repository covers all five required items; every claim below points at code you
can run.

Start here: [README.md](README.md) Â· run it in 30 seconds, no API keys needed.

---

## 1. Custom AI Automation Workflow â€” *build 1 complete workflow*

**`app/workflow.py` â€” `inbound_lead_triage`, seven steps, end to end.**
...
| 4 | `score` | Deterministic 0â€“100 score â†’ `hot` / `warm` / `cold` / `archived`, with the reasoning recorded per lead. |
...
full persisted trace, and **replay** â€” any stored payload can be re-run through the current
...
## 2. Third-Party API Integration â€” *integrate 1 REST API or webhook*
...
- **Inbound webhook** â€” `POST /webhook/inbound`, HMAC-SHA256 with the timestamp inside the MAC,
  a Â±300s replay window and constant-time comparison. Five rejection cases are unit-tested.
- **CRM (REST)** â€” `POST {CRM_BASE_URL}/contacts` with bearer auth.
- **Notification webhook** â€” Slack / Discord / Teams / any HTTPS endpoint (the payload field is
  chosen per provider).
- **Outbound event webhook** â€” signed with the same scheme we verify inbound, so downstream
  systems can trust it.

`scripts/mock_crm_server.py` implements all three outbound endpoints locally â€” including
verifying our outbound signature â€” so the live path is demonstrable with no third-party
account. Verified working end to end: CRM `HTTP 200`, notification delivered, outbound webhook
`signature_verified: true`.

---

## 3. Custom Dashboard â€” *develop 1 functional dashboard*

`http://127.0.0.1:5000` â€” a live control room, not a mockup.

- Six KPIs: leads triaged, hot leads, automation rate, average end-to-end handling time, spam
  filtered, AI spend (tokens Ã— list price).
...
Built with hand-rolled SVG â€” no chart library, no CDN, no external fonts. Light and dark are
...
Screenshots: [light](docs/dashboard-light.png) Â· [dark](docs/dashboard-dark.png) Â·
[workflow trace](docs/run-trace.png).

---

## 4. Real Client Automation Flow â€” *showcase 1 real client automation or a similar self-developed project*

FlowPilot is the **self-developed project** submitted under this item. The write-up â€”
the problem it solves, the before/after operating model, the decisions I'd defend in a review,
and honest limits â€” is in **[docs/CASE_STUDY.md](docs/CASE_STUDY.md)**.
...
| | Link |
|---|---|
| **Repository** | https://github.com/kanishqtanwar35-hub/flowpilot-ai-automation |
| **Live demo** | https://kanishqtanwar35-hub.github.io/flowpilot-ai-automation/demo.html â€” the real dashboard with real run data and no backend. Charts, filters, table views and the full workflow trace all work. |
| **What it does, in plain language** | [docs/WHAT_IT_DOES.md](docs/WHAT_IT_DOES.md) â€” the problem, the use case, a worked example |
| **Run it locally** | `pip install -r requirements.txt && python scripts/seed_demo.py --reset && python run.py` |

The repo is live and GitHub Pages serves the demo from `main` â†’ `/docs`. To ship a change:

```bash
python scripts/scan_secrets.py --install-hook   # once â€” blocks any commit containing a secret
python -m pytest -q                             # 33 tests
python scripts/build_static_demo.py             # refresh docs/demo.html from the live dashboard
git add . && git commit -m "â€¦" && git push
```
...
| Asked for | Where |
|---|---|
| AI Agent / OpenAIâ€“Gemini integration | Claude (`claude-opus-5`) via the Anthropic SDK with a strict JSON schema, retries and an automatic rule-engine fallback â€” `app/ai.py` |
| AI Dashboard | the control room above, including live AI cost/token accounting per run |
| Custom CRM | lead store, scoring, ownership, queues and SLA tracking â€” `app/db.py`, `app/workflow.py`, plus a REST CRM push |
...
- No `ANTHROPIC_API_KEY` was available in the environment where this was built, so the Claude
  ...
  fallback is what produced the seeded data â€” which is why the dashboard reports AI spend as $0.
...
The key is never in the repo, the logs, the database, the API, or the published demo â€” see
...
**Tests:** `python -m pytest -q` â†’ 33 passing.
