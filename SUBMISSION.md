...
| **Live demo** | https://kanishqtanwar35-hub.github.io/flowpilot-ai-automation/demo.html — the real dashboard with real run data and no backend. Charts, filters, table views and the full workflow trace all work. |
...
The repo is live and GitHub Pages serves the demo from `main` → `/docs`. To ship a change:

```bash
python scripts/scan_secrets.py --install-hook   # once — blocks any commit containing a secret
...
**Tests:** `python -m pytest -q` → 33 passing.
